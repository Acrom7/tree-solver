...
function printLevel(node, x, level) {
  ...
  if (level === 0) {
    ...
    let r = printLevel(node.left, x - node.edgeLength - 1, level - node.edgeLength - 1)
    return r + printLevel(node.right, x + node.edgeLength + 1, level - node.edgeLength - 1)
  }
}
...
function printASCIITree(tree) {
  ...
  for (let i = 0; i < root.height; i++) {
    printNext = 0
    result += printLevel(root, -minimumX, i)
    result += ('\n')
  }
